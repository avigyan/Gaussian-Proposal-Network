...
class DeepLesionDataset:
    ...
    def load_img(self, idx):
        ...
        img = np.stack([img_prev, img_cur, img_next], axis=2) - 32768 #2) - 32768
    ...
    def get_tf_dataset(self, batch_size=1, shuffle=False):
        """
        ...
        dataset = tf.data.Dataset.from_generator(
            generator,
            output_signature=(
                tf.TensorSpec(shape=(3, self._cfg['MAX_SIZE'], self._cfg['MAX_SIZE']), dtype=tf.float32),
                tf.TensorSpec(shape=(self._cfg['MAX_NUM_GT_BOXES'], 4), dtype=tf.float32),
                tf.TensorSpec(shape=(self._cfg['MAX_NUM_GT_BOXES'], 8), dtype=tf.float32),
            )
        )
